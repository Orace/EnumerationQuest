...
namespace EnumerationQuest.Tests
{
    ...
    {
        [TestCaseSource(nameof(MinByTestCases))]
        public Result MinByTest<TSource, TKey>(TSource _0, TKey _1, IEnumerable<TSource> source, Func<TSource, TKey> keySelector)
        {
            return Result.Evaluate(() => source.GetMinBy(keySelector).Deconstruct());
        }

        public static IEnumerable<object> MinByTestCases()
        {
        ...
        [TestCaseSource(nameof(MinByWithComparerTestCases))]
        public Result MinByWithComparerTest<TSource, TKey>(TSource _0, TKey _1, IEnumerable<TSource> source, Func<TSource, TKey> keySelector, IComparer<TKey> comparer)
        {
            return Result.Evaluate(() => source.GetMinBy(keySelector, comparer).Deconstruct());
        }

        public static IEnumerable<object> MinByWithComparerTestCases()
        {
            ...
            yield return new TestCaseData("", "", new[] { "X", "A", null, "B" }, IdOf<string>(), c) { ExpectedResult = Result.FromValue<string?>("A"), TestName = "Min is first" };
            yield return new TestCaseData("", "", new[] { "A", "X", null, "B" }, IdOf<string>(), c) { ExpectedResult = Result.FromValue<string?>("A"), TestName = "Min is in the middle" };
            yield return new TestCaseData("", "", new[] { "B", "A", null, "X" }, IdOf<string>(), c) { ExpectedResult = Result.FromValue<string?>("A"), TestName = "Min is last" };
