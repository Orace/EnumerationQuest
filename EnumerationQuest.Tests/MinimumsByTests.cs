...
{
    public class MinimumsByTests
    {
        [TestCaseSource(nameof(MinimumsByTestCases))]
        public Result MinimumsByTest<TSource, TKey>(TSource _0, TKey _1, IEnumerable<TSource> source, Func<TSource, TKey> keySelector)
        {
            return Result.Evaluate(() => Format(source.GetMinimumsBy(keySelector).Deconstruct()));
        }

        public static IEnumerable<object> MinimumsByTestCases()
        {
            ...
            yield return new TestCaseData(0, 0, new[] { 42, 100, 101 }, IdOf<int>()) { ExpectedResult = Result.FromValue("42"), TestName = "Minimums is first" };
            yield return new TestCaseData(0, 0, new[] { 100, 42, 101 }, IdOf<int>()) { ExpectedResult = Result.FromValue("42"), TestName = "Minimums is in the middle" };
            yield return new TestCaseData(0, 0, new[] { 110, 100, 42 }, IdOf<int>()) { ExpectedResult = Result.FromValue("42"), TestName = "Minimums is last" };

            // nan. For Enumerable.Minimums, nan are not ignored (while for max they are). See: https://codeblog.jonskeet.uk/2011/01/09/reimplementing-linq-to-objects-part-29-min-minimums/
            yield return new TestCaseData(0d, 0d, new[] { double.NaN }, IdOf<double>()) { ExpectedResult = Result.FromValue("NaN"), TestName = "One nan" };
            yield return new TestCaseData(0d, 0d, new[] { double.NaN, 0 }, IdOf<double>()) { ExpectedResult = Result.FromValue("NaN"), TestName = "One nan at the beginning" };
            yield return new TestCaseData(0d, 0d, new[] { 0, double.NaN }, IdOf<double>()) { ExpectedResult = Result.FromValue("NaN"), TestName = "One nan at the end" };
            ...
            yield return new TestCaseData(0d, 0d, new[] { double.NaN, double.NaN, 0 }, IdOf<double>()) { ExpectedResult = Result.FromValue("NaN|NaN"), TestName = "Two nan, value at the end" };
            yield return new TestCaseData(0d, 0d, new[] { double.NaN, 0, double.NaN }, IdOf<double>()) { ExpectedResult = Result.FromValue("NaN|NaN"), TestName = "Two nan, value in the middle" };
            yield return new TestCaseData(0d, 0d, new[] { 0, double.NaN, double.NaN }, IdOf<double>()) { ExpectedResult = Result.FromValue("NaN|NaN"), TestName = "Two nan, value at the beginning" };
            ...
            yield return new TestCaseData("", "", new[] { "X", "A", null, "B" }, IdOf<string>()) { ExpectedResult = Result.FromValue("A"), TestName = "Minimums is first" };
            yield return new TestCaseData("", "", new[] { "A", "X", null, "B" }, IdOf<string>()) { ExpectedResult = Result.FromValue("A"), TestName = "Minimums is in the middle" };
            yield return new TestCaseData("", "", new[] { "B", "A", null, "X" }, IdOf<string>()) { ExpectedResult = Result.FromValue("A"), TestName = "Minimums is last" };

            // many minimums
            var _ = new EqualsMock("_");
            var a = new EqualsMock("A");
            var b = new EqualsMock("B");
            yield return new TestCaseData(_, _, new[] { a, b }, IdOf<EqualsMock>()) { ExpectedResult = Result.FromValue("Mock A|Mock B"), TestName = "Many minimums" };
        }

        [TestCaseSource(nameof(MinimumsByWithComparerTestCases))]
        public Result MinimumsByWithComparerTest<TSource, TKey>(TSource _0, TKey _1, IEnumerable<TSource> source, Func<TSource, TKey> keySelector, IComparer<TKey> comparer)
        {
            return Result.Evaluate(() => Format(source.GetMinimumsBy(keySelector, comparer).Deconstruct()));
        }

        public static IEnumerable<object> MinimumsByWithComparerTestCases()
        {
            ...
            yield return new TestCaseData(0, 0, new[] { 42, 100, 101 }, IdOf<int>(), c) { ExpectedResult = Result.FromValue("42"), TestName = "Minimums is first" };
            yield return new TestCaseData(0, 0, new[] { 100, 42, 101 }, IdOf<int>(), c) { ExpectedResult = Result.FromValue("42"), TestName = "Minimums is in the middle" };
            yield return new TestCaseData(0, 0, new[] { 110, 100, 42 }, IdOf<int>(), c) { ExpectedResult = Result.FromValue("42"), TestName = "Minimums is last" };
            ...
            yield return new TestCaseData(0d, 0d, new[] { double.NaN, 0 }, IdOf<double>(), c) { ExpectedResult = Result.FromValue("NaN"), TestName = "One nan at the beginning" };
            yield return new TestCaseData(0d, 0d, new[] { 0, double.NaN }, IdOf<double>(), c) { ExpectedResult = Result.FromValue("NaN"), TestName = "One nan at the end" };
            ...
            yield return new TestCaseData(0d, 0d, new[] { double.NaN, double.NaN, 0 }, IdOf<double>(), c) { ExpectedResult = Result.FromValue("NaN|NaN"), TestName = "Two nan, value at the end" };
            yield return new TestCaseData(0d, 0d, new[] { double.NaN, 0, double.NaN }, IdOf<double>(), c) { ExpectedResult = Result.FromValue("NaN|NaN"), TestName = "Two nan, value in the middle" };
            yield return new TestCaseData(0d, 0d, new[] { 0, double.NaN, double.NaN }, IdOf<double>(), c) { ExpectedResult = Result.FromValue("NaN|NaN"), TestName = "Two nan, value at the beginning" };
            ...
            yield return new TestCaseData("", "", new[] { "X", "A", null, "B" }, IdOf<string>(), c) { ExpectedResult = Result.FromValue("A"), TestName = "Minimums is first" };
            yield return new TestCaseData("", "", new[] { "A", "X", null, "B" }, IdOf<string>(), c) { ExpectedResult = Result.FromValue("A"), TestName = "Minimums is in the middle" };
            yield return new TestCaseData("", "", new[] { "B", "A", null, "X" }, IdOf<string>(), c) { ExpectedResult = Result.FromValue("A"), TestName = "Minimums is last" };
            ...
            var a = new EqualsMock("A");
            var b = new EqualsMock("B");
            yield return new TestCaseData(_, _, new[] { a, b }, IdOf<EqualsMock>(), c) { ExpectedResult = Result.FromValue("Mock A|Mock B"), TestName = "Many minimums" };
